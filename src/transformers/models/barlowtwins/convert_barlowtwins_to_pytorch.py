...
"""Convert BarlowTwins checkpoints from timm."""
...
def convert_weight_and_push(name: str, config: BarlowTwinsConfig, save_directory: Path, push_to_hub: bool = True):
    ...
    with torch.no_grad():
        from_model = torch.hub.load("facebookresearch/barlowtwins:main", name).eval()
...
def convert_weights_and_push(save_directory: Path, model_name: str = None, push_to_hub: bool = True):
    ...
    if model_name:
        if model_name in names_to_config:
            convert_weight_and_push(model_name, names_to_config[model_name], save_directory, push_to_hub=False)
            return names_to_config[model_name], expected_shape
        else:
            raise ValueError(f"Model name '{model_name}' not recognized. Supported names: {list(names_to_config.keys())}")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "The name of the model you wish to convert, it must be one of the supported barlowtwins* architecture,"
            " currently: resnet50 "
        ),
    )
    parser.add_argument(
